...
We approach $\lambda$-models using the notion of a $C$-monoid, as defined by Lambek and Scott.

\begin{definition}
  \label{def:C-monoid}
  %
  A \emph{$C$-monoid} is a monoid $(C, {\cdot}, 1)$ with elements $\pi_1, \pi_2, \varepsilon \in C$,
  a binary operation $\pair{{-},{-}} : C \times C \to C$ and unary operation ${-}^* : C \to C$
  ...
  \begin{align}
    ...
    \varepsilon \pair{h^* \, \pi_1, \pi_2} &= h, \\
    (\varepsilon \pair{k \, \pi_1, \pi_2})^* &= k.
  \end{align}
\end{definition}
...
\begin{theorem}
  \label{def:C-monoid-is-PCA}
  A $C$-monoid induces the structure of a combinatory algebra.
\end{theorem}
